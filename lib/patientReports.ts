...
function detectHospice(row: CsvRow): boolean {
  ...
  const hospiceStatus =
    status === "hospice" ||
    status === "active hospice";

  const hospicePayor =
    payor.includes("hospice");

  const hospiceSource =
    source.includes("hospice");

  return hospiceStatus || hospicePayor || hospiceSource;
}
...
function buildPatientId(row: CsvRow): string {
  ...
  return `name__${fullName || "unknown"}__${dob || "unknown"}`;
}

function unique(values: string[]): string[] {
  return Array.from(
    new Set(values.map((v) => normalizeWhitespace(v)).filter(Boolean))
...
export async function mergePatientsFromReportUpload(params: {
  ...
  const { db, reportType, sourceFileName, rows } = params;
  ...
  const arrayUploadedAt = Timestamp.now();
  ...
  for (let i = 0; i < patientIds.length; i += 200) {
    ...
    for (const patientId of chunkIds) {
      ...
      const nextDoc: PatientDoc = {
        ...
        hospice: extracted.some((item) => item.hospice) || existing?.hospice || false,
        ...
        reportTypes: unique([...(existing?.reportTypes ?? []), reportType]),
