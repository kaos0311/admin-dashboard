...
export function UploadBatchActions({
  ...
  const hasQueuedFiles = queueCount > 0;
  const hasFailedUploads = failedCount > 0;
  const hasCompletedUploads = completedCount > 0;

  const startDisabled = disabled || uploading || !hasQueuedFiles;
  const retryDisabled = disabled || uploading || !hasFailedUploads;
  const clearDisabled = uploading || !hasCompletedUploads;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={`Retry ${failedCount} failed upload${failedCount === 1 ? "" : "s"}`}
      >
        <RotateCcw className="h-4 w-4" aria-hidden="true" />
        Retry Failed ({failedCount.toLocaleString()})
      </button>
      ...
      <button
        ...
        aria-label={`Clear ${completedCount} completed upload${completedCount === 1 ? "" : "s"}`}
      >
        <Trash2 className="h-4 w-4" aria-hidden="true" />
        Clear Completed ({completedCount.toLocaleString()})
      </button>
