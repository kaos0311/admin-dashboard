export const tiles = {
  ...
  label:
    "text-xs font-semibold uppercase tracking-[0.16em] text-slate-400 light:text-slate-500",

  title:
    "text-base font-bold tracking-tight text-white sm:text-lg light:text-slate-950",

  value:
    "mt-3 text-3xl font-black tracking-tight text-white light:text-slate-950",

  helper:
    "mt-2 text-xs text-slate-400 light:text-slate-600",

  description:
    "text-sm leading-6 text-slate-400 light:text-slate-600",

  badge:
    "inline-flex items-center rounded-full border border-white/10 bg-white/[0.08] px-3 py-1 text-[0.68rem] font-black uppercase tracking-[0.2em] text-slate-200 light:border-slate-200 light:bg-white/70 light:text-slate-700",

  gridMetrics:
    "grid auto-rows-fr items-stretch gap-4 sm:grid-cols-2 xl:grid-cols-4 [&>*]:h-full",

  gridSections:
    "grid auto-rows-fr items-stretch gap-4 lg:grid-cols-2 xl:grid-cols-4 [&>*]:h-full",

  gridTwo:
    "grid auto-rows-fr items-stretch gap-4 lg:grid-cols-2 [&>*]:h-full",
} as const;
