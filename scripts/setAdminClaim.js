...
function loadServiceAccount() {
  ...
  if (!filePath) {
    throw new Error(
      `Missing Firebase credentials. Set FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY, or place serviceAccountKey.json in one of: ${possiblePaths.join(", ")}`
    );
  }
...
function getEmail() {
  ...
  if (!email || !email.trim()) {
    throw new Error("Provide email: node scripts/bootstrapAdmin.js you@email.com");
  }
...
function getDisplayName() {
  return readEnv("BOOTSTRAP_ADMIN_DISPLAY_NAME") || "Admin";
}

function getTempPassword() {
  return readEnv("BOOTSTRAP_ADMIN_PASSWORD") || DEFAULT_TEMP_PASSWORD;
}
...
async function run() {
  ...
  if (created) {
    ...
    console.log("\nChange it after first login. Default passwords are how raccoons with laptops become executives.");
  } else {
