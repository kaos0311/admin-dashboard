...
const DEFAULT_FILE = "C:/Users/pboyl/Downloads/Active Rentals (1).csv";
const SOURCE_FILE = FILE_ARG ?? DEFAULT_FILE;
const IMPORT_ID = `manual-active-rentals-${new Date()
  .toISOString()
  .replace(/[:.]/g, "-")}`;
...
async function main() {
  if (!fs.existsSync(SOURCE_FILE)) {
    throw new Error(`Active Rentals CSV not found: ${SOURCE_FILE}`);
  }

  const firestoreAdmin = await initializeFirebaseAdmin();
  const { processActiveRentals } = await import(
    "../functions/src/imports/processors/activeRentals/activeRentalsProcessor"
  );

  const rows = parseRows(SOURCE_FILE);
  const uniquePatients = new Set(rows.map((row) => String(row.PtID ?? "").trim()).filter(Boolean));
  const uniqueItems = new Set(rows.map((row) => String(row.ItemID ?? "").trim()).filter(Boolean));
  ...
  for (let offset = 0; offset < rows.length; offset += CHUNK_SIZE) {
    ...
    const result = await processActiveRentals(IMPORT_ID, chunk, offset);

    processedCount += result.processedCount;
    skippedCount += result.skippedCount;
    issueCount += result.issueCount;
    console.log(
      `Processed ${Math.min(offset + CHUNK_SIZE, rows.length)} / ${rows.length}`
    );
  }
