...
export function RecentActivityCard() {
  return (
    <aside className={`${glass.inset} p-4`}>
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-3">
        <p className={`text-sm leading-6 ${typography.bodyMuted}`}>
          Activity logging should be wired to{" "}
          <span className="text-slate-200">auditLogs</span> when settings
          updates are finalized through admin-only operations.
        </p>
      </div>
    </aside>
  );
